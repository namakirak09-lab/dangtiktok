...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { account_id, session_state, client_profile } = await req.json()
    ...
    const encrypted = await encryptJson(session_state)
    ...
    const { error: accountUpdateError } = await db.from('tiktok_accounts').update({
      ...
      capabilities: { session_source: 'local_chrome', imported_at: now },
      last_health_at: now,
    }).eq('id', account_id)
